...
const Perfil = () => {
  ...
  const guardarDatos = () => {
    ...
    const [day, month, year] = datosPerfil.fecha_nac.split('/');
    const fecha_nac_formateada = new Date(`${year}-${month}-${day}`).toISOString().split('T')[0];
    const datosActualizados = { ...datosPerfil, fecha_nac: fecha_nac_formateada };
    const config = {
      headers: {
        Authorization: token
      }
